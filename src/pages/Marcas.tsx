...
export default function Marcas() {
  ...
  const filtered = marcas.filter((c) => {
  ...
  async function onSubmit(values: any) {
    try {
      ...
      if (editing) {
        const updated = await updateMarca(editing.id, { nombre: String(values.nombre).trim(), descripcion: String(values.descripcion || '') });
        setMarcas((prev) => prev.map((p) => (p.id === editing.id ? updated : p)));
        toast.success('Marca actualizada');
        setEditing(null);
      } else {
        const created = await createMarca({ nombre: String(values.nombre).trim(), descripcion: String(values.descripcion || '') });
      ...
      console.error(err);
      toast.error(parseApiError(err) || err?.message || 'Error al guardar marca');
    }
  }
  ...
  const remove = async (c: any) => {
    try {
      ...
      setMarcas((prev) => prev.filter((x) => x.id !== c.id));
      toast.success('Marca eliminada');
    } catch (err: any) {
      console.error('Error eliminando marca', err);
      toast.error(parseApiError(err) || err?.message || 'Error eliminando marca');
    } finally {
